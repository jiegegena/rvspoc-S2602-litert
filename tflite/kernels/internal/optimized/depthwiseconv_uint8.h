  ...
  static void Run(int num_output_pixels, int input_depth, int depth_multiplier,
    ...
    for (int outp = 0; outp < num_output_pixels; outp++) {
      for (int d = 0; d < input_depth; d++) {
        // Load input value, widen, add offset.
        const uint8_t in_u8 = input_ptr[d];
        const int16_t in_s16 = static_cast<int16_t>(in_u8) + input_offset;
        // Load 2 filter values for this input channel.
        const uint8_t f0_u8 = filter_ptr[d * 2];
        const uint8_t f1_u8 = filter_ptr[d * 2 + 1];
        const int16_t f0_s16 = static_cast<int16_t>(f0_u8) + filter_offset;
        const int16_t f1_s16 = static_cast<int16_t>(f1_u8) + filter_offset;
        // Multiply-accumulate into int32.
...
struct QuantizedDepthwiseConvKernel<false, 8, 1> {
  static void Run(int num_output_pixels, int input_depth, int depth_multiplier,
                  ...
                  int input_ptr_increment, const uint8_t* filter_ptr,
                  int16_t filter_offset, int32_t* acc_buffer_ptr) {
    // vl=8: valid for LMUL=1 (≥8), LMUL=2 (≥8), LMUL=4 (≥8).
    // Requires VLEN >= 64 (all practical RVV implementations).
    const size_t vl = __riscv_vsetvl_e8m1(8);
    // Load filter once, widen: u8(m1) → u16(m2) → i16(m2) + offset.
    vuint8m1_t filter_u8 = __riscv_vle8_v_u8m1(filter_ptr, vl);
    vint16m2_t filter_s16 = __riscv_vreinterpret_v_u16m2_i16m2(
        __riscv_vzext_vf2_u16m2(filter_u8, vl));
    filter_s16 = __riscv_vadd_vx_i16m2(filter_s16, filter_offset, vl);

    int outp = 0;
    for (; outp <= num_output_pixels - 2; outp += 2) {
      // Pixel 0: load acc(i32,m4), input(u8,m1→i16,m2), wmac.
      vint32m4_t acc0 = __riscv_vle32_v_i32m4(acc_buffer_ptr, vl);
      vint16m2_t in0 = __riscv_vreinterpret_v_u16m2_i16m2(
          __riscv_vzext_vf2_u16m2(__riscv_vle8_v_u8m1(input_ptr, vl), vl));
      ...
      vint16m2_t in1 = __riscv_vreinterpret_v_u16m2_i16m2(
          __riscv_vzext_vf2_u16m2(
              __riscv_vle8_v_u8m1(input_ptr + input_ptr_increment, vl), vl));
    ...
    for (; outp < num_output_pixels; outp++) {
      ...
      vint16m2_t in = __riscv_vreinterpret_v_u16m2_i16m2(
          __riscv_vzext_vf2_u16m2(__riscv_vle8_v_u8m1(input_ptr, vl), vl));
...
struct QuantizedDepthwiseConvKernel<false, 4, 1> {
  static void Run(int num_output_pixels, int input_depth, int depth_multiplier,
    ...
    int32_t f[4];
    for (int i = 0; i < 4; i++) {
      f[i] = static_cast<int32_t>(
          static_cast<int16_t>(filter_ptr[i]) + filter_offset);
    }
    int outp = 0;
    for (; outp <= num_output_pixels - 4; outp += 4) {
      for (int p = 0; p < 4; p++) {
        const uint8_t* in = input_ptr + p * input_ptr_increment;
...
struct QuantizedDepthwiseConvKernel<false, 2, 1> {
  static void Run(int num_output_pixels, int input_depth, int depth_multiplier,
                  ...
                  int input_ptr_increment, const uint8_t* filter_ptr,
                  int16_t filter_offset, int32_t* acc_buffer_ptr) {
    const int32_t f0 = static_cast<int32_t>(
        static_cast<int16_t>(filter_ptr[0]) + filter_offset);
    const int32_t f1 = static_cast<int32_t>(
        static_cast<int16_t>(filter_ptr[1]) + filter_offset);
    int outp = 0;
    for (; outp <= num_output_pixels - 4; outp += 4) {
      const uint8_t* in0 = input_ptr;
      const uint8_t* in1 = input_ptr + input_ptr_increment;
      const uint8_t* in2 = input_ptr + 2 * input_ptr_increment;
      const uint8_t* in3 = input_ptr + 3 * input_ptr_increment;
...
struct QuantizedDepthwiseConvKernel<true, 8, 1> {
  static void Run(int num_output_pixels, int input_depth, int depth_multiplier,
    ...
    int32_t f[8];
    for (int i = 0; i < 8; i++)
      f[i] = static_cast<int32_t>(static_cast<int16_t>(filter_ptr[i]) + filter_offset);
    for (int outp = 0; outp < num_output_pixels; outp++) {
      for (int i = 0; i < 8; i++)
        acc_buffer_ptr[i] += (static_cast<int32_t>(static_cast<int16_t>(input_ptr[i]) + input_offset)) * f[i];
      input_ptr += input_ptr_increment;
      acc_buffer_ptr += 8;
    }
  }
...
struct QuantizedDepthwiseConvKernel<true, 2, 1> {
  static void Run(int num_output_pixels, int input_depth, int depth_multiplier,
                  ...
                  int input_ptr_increment, const uint8_t* filter_ptr,
                  int16_t filter_offset, int32_t* acc_buffer_ptr) {
    const int32_t f0 = static_cast<int32_t>(static_cast<int16_t>(filter_ptr[0]) + filter_offset);
    const int32_t f1 = static_cast<int32_t>(static_cast<int16_t>(filter_ptr[1]) + filter_offset);
    for (int outp = 0; outp < num_output_pixels; outp++) {
      acc_buffer_ptr[0] += (static_cast<int32_t>(static_cast<int16_t>(input_ptr[0]) + input_offset)) * f0;
      acc_buffer_ptr[1] += (static_cast<int32_t>(static_cast<int16_t>(input_ptr[1]) + input_offset)) * f1;
      input_ptr += input_ptr_increment;
      acc_buffer_ptr += 2;
    }
  }
...
struct QuantizedDepthwiseConvKernel<true, 1, 2> {
  static void Run(int num_output_pixels, int input_depth, int depth_multiplier,
    ...
    const int32_t f1 = static_cast<int32_t>(static_cast<int16_t>(filter_ptr[1]) + filter_offset);
    for (int outp = 0; outp < num_output_pixels; outp++) {
      int32_t in = static_cast<int32_t>(static_cast<int16_t>(input_ptr[0]) + input_offset);
...
struct QuantizedDepthwiseConvKernel<false, 2, 2> {
  static void Run(int num_output_pixels, int input_depth, int depth_multiplier,
    ...
    for (int i = 0; i < 4; i++)
      f[i] = static_cast<int32_t>(static_cast<int16_t>(filter_ptr[i]) + filter_offset);
    for (int outp = 0; outp < num_output_pixels; outp++) {
      int32_t in0 = static_cast<int32_t>(static_cast<int16_t>(input_ptr[0]) + input_offset);
      int32_t in1 = static_cast<int32_t>(static_cast<int16_t>(input_ptr[1]) + input_offset);
...
struct QuantizedDepthwiseConvKernel<false, 4, 4> {
  static void Run(int num_output_pixels, int input_depth, int depth_multiplier,
    ...
    for (int outp = 0; outp < num_output_pixels; outp++) {
      for (int k = 0; k < 4; k++) {
        int32_t in = static_cast<int32_t>(static_cast<int16_t>(input_ptr[k]) + input_offset);
...
struct QuantizedDepthwiseConvKernel<true, 0, 3> {
  static void Run(int num_output_pixels, int input_depth, int depth_multiplier,
    ...
    for (int outp = 0; outp < num_output_pixels; outp++) {
      for (int d = 0; d < input_depth; d++) {
        int32_t in = static_cast<int32_t>(static_cast<int16_t>(input_ptr[d]) + input_offset);
        acc_buffer_ptr[d * 3] += in * (static_cast<int32_t>(static_cast<int16_t>(filter_ptr[d * 3]) + filter_offset));
        acc_buffer_ptr[d * 3 + 1] += in * (static_cast<int32_t>(static_cast<int16_t>(filter_ptr[d * 3 + 1]) + filter_offset));
        acc_buffer_ptr[d * 3 + 2] += in * (static_cast<int32_t>(static_cast<int16_t>(filter_ptr[d * 3 + 2]) + filter_offset));
      }
      input_ptr += input_ptr_increment;
      acc_buffer_ptr += output_depth;
    }
...
#elif defined(USE_RVV)
        // RVV requantization: fall back to scalar for correctness.
        // The vmulh-based approach loses precision for output_shift < 0
        // because it discards low 32 bits before adding rounding.
        // This is NOT the performance bottleneck (accumulation is), so scalar is fine.
        (void)shift_left;
        (void)multiplier_power_of_two;
